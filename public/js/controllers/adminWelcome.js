...
function AdminWelcome(rsvps, $scope, $http) {

    $scope.welcomeStatus = {};
    rsvps.forEach(function (rsvp) {
        $scope.welcomeStatus[rsvp._id] = rsvp.meta.welcomed ? ':-)' : '';
    });
    ...
    var sendWelcomeEmail = function(rsvp, callback) {
        ...
        $http.put("/admin/data/email/welcome", rsvp)
        .success(function() {
...
AdminWelcome.$inject = ['rsvps', '$scope', '$http'];
