...
Weekend.Services.surveySession = function (survey) {
    ...
    var today = new Date();
    var sessionKey = 'swing-and-blues-weekend-2015-surveySession';
    ...
    for (var prop in survey) {
        if (!session[prop]) {
            session[prop] = survey[prop];
        }
    }

    return session;
};
Weekend.Services.surveySession.$inject = ['survey'];
angular.module('project').service('surveySession', Weekend.Services.surveySession);