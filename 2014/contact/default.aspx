<!doctype html>
<html lang="en">
<head>
	<title>Contact: <!--#include virtual = "/inc/event-name.aspx" --></title>
	<meta name="description" content="Contact the organizers of Corvallis Swing and Blues Weekend."/>
<!--#include virtual = "/inc/head-common.html" -->

<style>
.container {
	max-width: 940px;
	width: auto;
}

.left-align {	
	margin-left: 0;	
}

</style>

<!--#include virtual = "/inc/head-analytics.aspx" -->
</head>
<body>
<!--#include virtual = "/inc/header.html" -->	
	<div class="container">
	<div class="max12">
		<h1>Contact us</h1>

		<p>This event is organized by the same people you know from Corvallis Swing and OSU Blues, so say hello to Avery, Casey, Catherine, Eileen, Ellen, Hannah, Jeff, Kristin, Laurel, Lindsey, Martin, Miranda, Phil, Renee, Sheena, or Zach the next time you bump into them at a dance.</p>
		
		<div class="max12" id="contact">
			<div class="row">
				<div class="max4 left-align" id="by-email">
					<h4>Email</h4>

					<p>Please use the Corvallis Swing
					form: <a href="http://corvallisswing.com/contact/">http://corvallisswing.com/contact/</a>.</p>
				</div>

				<div class="max4 left-align" id="by-mail">
					<h4>Correspondence</h4>
						
					<address>
						Corvallis Swing Dance Society<br/>
						PO Box 2485<br/>
						Corvallis, OR 97339
					</address>
				</div>

				<div class="max4 left-align" id="facebook">
					<h4>Facebook</h4>
 					<p>We have <a href="https://www.facebook.com/events/167613050113178/">an event</a> to
					save the date.</p> 

					Groups: 
					<ul>						
						<li><a href="https://www.facebook.com/groups/corvallisswing/">Corvallis Swing</a></li>
						<li><a href="https://www.facebook.com/groups/OSUBlues/">OSU Blues</a></li>	
					</ul>
				</div>
			</div>

 			<div class="row">
				<div id="payment" class="max12 left-align">
					<h4>Payment</h4>
						<p>Already made a <a href="/rsvp/">reservation</a>? Our payment page is <a href="/rsvp/#/payment/">right here</a>.</p>
				</div>
			</div>
		</div>
	</div>
	</div>
<!--#include virtual = "/inc/footer.html" -->	
</body>
</html>